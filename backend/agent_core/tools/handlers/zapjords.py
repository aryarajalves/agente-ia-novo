...
async def handle_zapjords_handoff(db, context_variables, target_tool, is_human, func_args_str, history, config_id):
    try:
        ...
        # Sincronizar Etiquetas no ZapJords/ZapVoice
        if zv_url and zv_token and client_id and conversation_id:
            from zapvoice_utils import sync_conversation_labels
            try:
                await sync_conversation_labels(zv_url, str(client_id), int(conversation_id), zv_token, labels_add, labels_remove)
            except Exception as e_sync:
                print(f"⚠️ Erro na sincronização de labels ZapJords: {e_sync}")
        ...
        new_support = SupportRequestModel(
            ...
            user_name=context_variables.get("contact_name") or "Usuário ZapJords",
            ...
            account_id=str(client_id) if client_id else None,
        ...
        import traceback
        traceback.print_exc()
        return f"Erro na automação ZapJords: {str(e)}"
