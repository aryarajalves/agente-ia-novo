...
async def run_migration():
    print("Iniciando migração de coluna qualified_by_agent_id nas tabelas de leads...")
    async with async_session() as db:
        try:
            # 1. Busca todas as tabelas de leads dinâmicas em webhook_configs
            ...
            for table in tables_to_migrate:
                if table:
                    print(f"Migrando tabela {table} (adicionando qualified_by_agent_id)...")
                    try:
                        await db.execute(text(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS qualified_by_agent_id INTEGER"))
                        await db.commit()
