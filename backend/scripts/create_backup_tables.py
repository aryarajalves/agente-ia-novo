...
def migrate():
    logger.info("🚀 Iniciando migração para tabelas de backup...")
    
    with engine_sync.begin() as conn:
        logger.info("🛠️ Criando tabelas (se não existirem)...")
        Base.metadata.create_all(conn, tables=[
            models.BackupConfigModel.__table__,
            models.BackupHistoryModel.__table__
        ])
        
        logger.info("✅ Tabelas de backup criadas com sucesso.")
