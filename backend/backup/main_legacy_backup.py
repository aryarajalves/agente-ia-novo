from fastapi import FastAPI, Depends, HTTPException, UploadFile, File, Form, Security, Request, Response, BackgroundTasks, Query
...
from config_store import AgentConfig, KnowledgeBase, KnowledgeItem, MODEL_INFO, USD_TO_BRL, get_model_pricing
...
from models import InteractionLog, AgentConfigModel, ToolModel, KnowledgeBaseModel, KnowledgeItemModel, SessionSummary, PromptDraftModel, FeedbackLog, GlobalContextVariableModel, UserModel, UnansweredQuestionModel, SupportRequestModel, TranscriptionTaskModel, TranscriptionFolder
...
async def get_current_user(token: str = Depends(APIKeyHeader(name="Authorization", auto_error=False))):
    ...
    try:
        ...
        # Validar o token com um pequeno 'leeway' (margem de erro) para evitar problemas de sincronização de relógio
        payload = jwt.decode(
            token, 
            SECRET_KEY, 
            algorithms=[ALGORITHM],
            options={"leeway": 60} # 60 segundos de tolerância
        )
        ...
        # Se expirar, vamos logar o tempo atual para ajudar no debug
...
# Configurações de CORS Restrito
...
async def upload_image_endpoint(file: UploadFile = File(...)):
    ...
    logger.info(f"Recebendo upload de imagem. Tamanho: {file.size if hasattr(file, 'size') else 'desconhecido'}")
    ...
    if s3_configured:
        try:
            ...
            file_obj.name = filename  # boto3 pode usar o atributo .name

            # Reutiliza o executor do s3_service para upload do bytes
            ...
            logger.warning("S3 upload ok mas não gerou URL pública — usando fallback local.")
        except Exception as e:
    ...
    logger.warning(f"⚠️ Imagem salva localmente (S3 não configurado): {image_url}")
...
class MessageRequest(BaseModel):
    ...
    image_url: str | None = None # NEW: For Vision Support
...
class ToolCreate(BaseModel):
    ...
    labels_to_add: str| None = None  # JSON list
    labels_to_remove: str | None = None  # JSON list
...
@app.delete("/knowledge-bases/batch-delete", dependencies=[Depends(verify_api_key)])
@app.post("/knowledge-bases/batch-delete", dependencies=[Depends(verify_api_key)])
async def batch_delete_knowledge_bases(request: BatchDeleteRequest, db: AsyncSession = Depends(get_db)):
    ...
    await db.execute(
        ...
        .where(KnowledgeBaseModel.id.in_(request.item_ids))
    )
    await db.commit()
...
async def background_s3_upload(local_path: str, s3_key: str, task_id: int, config_dict: dict):
    try:
        ...
        async with async_session() as db:
            ...
            if task:
                task.status = "FAILURE"
                task.error_message = f"Falha no upload para storage local: {str(e)}"
                await db.commit()
    finally:
        import os
...
@app.post("/knowledge-bases/generate-upload-url", dependencies=[Depends(verify_api_key)])
async def generate_upload_url(request: GenerateUploadUrlRequest, db: AsyncSession = Depends(get_db)):
    """Gera URL presigned e cria a task de transcrição."""
    import time
    try:
        from s3_service import s3_service
        ...
        # Gera URL PUT
        put_url = s3_service.generate_presigned_put_url(
            s3_key, 
            content_type=request.content_type, 
            expiration=3600
        )
        if not put_url:
            raise HTTPException(status_code=500, detail="Erro ao gerar URL de upload AWS/B2.")
        ...
        logger.info(f"✅ generate-upload-url: Task criada com ID={new_task.id} para '{request.filename}'")
        return {
            "url": put_url,
            "s3_key": s3_key,
            "task_id": new_task.id
        ...
        logger.error(f"Erro no endpoint generate_upload_url: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/knowledge-bases/confirm-upload", dependencies=[Depends(verify_api_key)])
async def confirm_upload_endpoint(request: ConfirmUploadRequest, db: AsyncSession = Depends(get_db)):
    """Confirma upload do arquivo para o S3 e dispara Celery para transcrição."""
    try:
        from tasks import process_transcription_task
        logger.info(f"🔍 confirm-upload: Buscando task_id={request.task_id}")
        task = await db.get(TranscriptionTaskModel, request.task_id)

        if not task:
            all_tasks = await db.execute(select(TranscriptionTaskModel))
            ids = [t.id for t in all_tasks.scalars().all()]
            logger.error(f"❌ confirm-upload: task_id={request.task_id} NÃO encontrada. Tasks no DB: {ids}")
            raise HTTPException(status_code=404, detail="Task não encontrada")
            
        logger.info(f"BACKEND: Confirmação de upload para task_id={request.task_id}. Disparando Celery...")
        process_transcription_task.delay(request.task_id, task.s3_key, request.config)
        
        return {"message": "Processamento de transcrição iniciado.", "status": "PENDING"}
    except Exception as e:
        logger.error(f"Erro em confirm-upload: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def transcribe_video_endpoint(
    ...
    """
    Novo fluxo:
    1. Salva arquivo localmente muito rápido.
    2. Retorna sucesso para o navegador instantaneamente (evita timeout).
    3. Upload para S3 ocorre em Background Task e dispara Celery.
    """
    try:
        import shutil
        import uuid
        import os
        import time
        # 1. Verificar Limite de 5GB
        max_size = int(os.getenv("MAX_CONTENT_LENGTH", 5368709120))
        content_length = request.headers.get("content-length")
        
        if content_length and int(content_length) > max_size:
            raise HTTPException(status_code=413, detail="Arquivo excede o limite de 5GB.")
        ...
        async with async_session() as db:
            logger.info(f"BACKEND: Criando registro de transcrição para {file.filename}")
            new_task = TranscriptionTaskModel(
                knowledge_base_id=kb_id,
            ...
            await db.refresh(new_task)
            task_id = new_task.id
            logger.info(f"BACKEND: Registro {task_id} criado. Disparando background task para S3...")

        # 4. Disparar Background Task
        background_tasks.add_task(background_s3_upload, local_path, s3_key, task_id, config_dict)

        return {
            "message": "Transcrição iniciada e adicionada à fila.",
            "task_id": task_id,
            "status": "PENDING"
        }
                
    except Exception as e:
        logger.error(f"Erro no endpoint de transcrição: {str(e)}", exc_info=True)
        if isinstance(e, HTTPException): raise e
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/transcription-tasks", dependencies=[Depends(verify_api_key)])
async def list_transcription_tasks(
    ...
    """Lista o histórico de transcrições com paginação."""
    # Evitar cache do navegador para o histórico
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    response.headers["Pragma"] = "no-cache"
    
    logger.info(f"📅 BACKEND: list_transcription_tasks chamado. Page={page}, Limit={limit}")
    
    offset = (page - 1) * limit

    # Query para contar o total
    count_stmt = select(func.count()).select_from(TranscriptionTaskModel).where(TranscriptionTaskModel.status != "PENDING")
    
    # Query paginada
    stmt = select(TranscriptionTaskModel).where(TranscriptionTaskModel.status != "PENDING")
    
    if folder_id:
        count_stmt = count_stmt.where(TranscriptionTaskModel.folder_id == folder_id)
        stmt = stmt.where(TranscriptionTaskModel.folder_id == folder_id)
    else:
        # Se folder_id for None (Raiz), às vezes queremos apenas os sem pasta ou todos?
        # O usuário disse "organizar em pastas", geralmente implica que as pastas filtram.
        # Vou assumir que se folder_id for fornecido, filtramos. Se for None, mostramos todos da raiz.
        pass
    ...
    result = await db.execute(
        stmt.order_by(TranscriptionTaskModel.created_at.desc())
        .offset(offset)
        .limit(limit)
    )
    tasks = result.scalars().all()
    
    logger.info(f"📅 BACKEND: Retornando {len(tasks)} tarefas de um total de {total_count}.")
...
@app.get("/transcription-folders", dependencies=[Depends(verify_api_key)])
async def list_transcription_folders(db: AsyncSession = Depends(get_db)):
    """Lista todas as pastas de organização de transcrições."""
    result = await db.execute(select(TranscriptionFolder).order_by(TranscriptionFolder.name))
    return result.scalars().all()

@app.post("/transcription-folders", dependencies=[Depends(verify_api_key)])
async def create_transcription_folder(request: TranscriptionFolderRequest, db: AsyncSession = Depends(get_db)):
    """Cria uma nova pasta de transcrições."""
...
@app.delete("/transcription-folders/{folder_id}", dependencies=[Depends(verify_api_key)])
async def delete_transcription_folder(folder_id: int, db: AsyncSession = Depends(get_db)):
    """Exclui uma pasta (os registros nela contidos voltam para a raiz)."""
...
@app.get("/transcription-tasks/{task_id}", dependencies=[Depends(verify_api_key)])
async def get_transcription_task(task_id: int, db: AsyncSession = Depends(get_db)):
    """Busca os detalhes de uma transcrição específica."""
    task = await db.get(TranscriptionTaskModel, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
    return task

@app.post("/transcription-tasks/bulk-delete", dependencies=[Depends(verify_api_key)])
async def bulk_delete_transcription_tasks(request: BulkDeleteTranscriptionRequest, db: AsyncSession = Depends(get_db)):
    """Remove múltiplos registros do histórico de uma só vez."""
    if not request.task_ids:
        return {"message": "Nenhum ID fornecido."}
    
    # Busca as tarefas para deletar os arquivos do S3
    result = await db.execute(
    ...
    for task in tasks:
        if task.s3_key:
            try:
                ...
                logger.warning(f"⚠️ Erro ao deletar arquivo S3 ({task.s3_key}) na exclusão em massa: {str(e)}")
...
@app.delete("/transcription-tasks/{task_id}", dependencies=[Depends(verify_api_key)])
async def delete_transcription_task(task_id: int, db: AsyncSession = Depends(get_db)):
    """Remove um registro do histórico."""
    task = await db.get(TranscriptionTaskModel, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
    
    # Se ainda tiver arquivo no S3 por algum erro, tenta limpar
    if task.s3_key:
        try:
            s3_service.delete_file(task.s3_key)
        except Exception as e:
            logger.warning(f"⚠️ Erro ao deletar arquivo S3 ({task.s3_key}): {str(e)}")
            # Continuamos para deletar o registro do banco mesmo se o S3 falhar
        
    await db.delete(task)
    await db.commit()
    return {"message": "Registro removido com sucesso."}

@app.patch("/transcription-tasks/{task_id}", dependencies=[Depends(verify_api_key)])
async def rename_transcription_task(task_id: int, request: TranscriptionRenameRequest, db: AsyncSession = Depends(get_db)):
    """Renomeia um registro do histórico de transcrições."""
    task = await db.get(TranscriptionTaskModel, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
    
    if not request.filename or not request.filename.strip():
        raise HTTPException(status_code=400, detail="O nome do arquivo não pode estar vazio.")
        
    task.filename = request.filename.strip()
    await db.commit()
    await db.refresh(task)
    
    return task

@app.patch("/transcription-tasks/{task_id}/move", dependencies=[Depends(verify_api_key)])
async def move_transcription_task(task_id: int, request: TranscriptionMoveRequest, db: AsyncSession = Depends(get_db)):
    """Move uma transcrição para uma pasta específica."""
    task = await db.get(TranscriptionTaskModel, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
    
    if request.folder_id:
        folder = await db.get(TranscriptionFolder, request.folder_id)
        if not folder:
...
@app.post("/transcription-tasks/manual", dependencies=[Depends(verify_api_key)])
async def create_manual_transcription(request: ManualTranscriptionRequest, db: AsyncSession = Depends(get_db)):
    """Cria uma transcrição manual fornecendo diretamente o texto."""
    new_task = TranscriptionTaskModel(
        filename=request.filename.strip(),
        result_text=request.text,
        status="SUCCESS", # Já nasce como sucesso pois o texto foi fornecido
...
@app.patch("/transcription-tasks/{task_id}/content", dependencies=[Depends(verify_api_key)])
async def update_transcription_content(task_id: int, request: TranscriptionContentUpdateRequest, db: AsyncSession = Depends(get_db)):
    """Atualiza o conteúdo de texto de uma transcrição existente."""
    task = await db.get(TranscriptionTaskModel, task_id)
    if not task:
        raise HTTPException(status_code=404, detail="Tarefa não encontrada")
    
    task.result_text = request.text
    await db.commit()
    await db.refresh(task)
    return task
...
async def process_transcription_endpoint(
    ...
    summary_meta = config.get("summary_metadata", "")
    qa_meta = config.get("qa_metadata", "")
    chunks_meta = config.get("chunks_metadata", "")
    ...
    try:
        ...
        if config.get("extractQA"):
            ...
            for item in qa_list:
                final_meta = f"{metadata_val} | {qa_meta}" if metadata_val and qa_meta else (qa_meta or metadata_val)
                items_to_add.append(KnowledgeItemModel(
                    knowledge_base_id=kb_id,
                    question=item.get("pergunta", "Questão extraída"),
                    answer=item.get("resposta", ""),
                    metadata_val=f"{final_meta} | Source: Transcrição AI" if final_meta else "Transcrição AI",
        ...
        if config.get("generateSummary"):
            try:
                ...
                if client:
                    ...
                    final_meta = f"{metadata_val} | {summary_meta}" if metadata_val and summary_meta else (summary_meta or metadata_val)
                    items_to_add.append(KnowledgeItemModel(
                        knowledge_base_id=kb_id,
                        question="Resumo da Transcrição",
                        answer=summary_text,
                        metadata_val=f"{final_meta} | Source: Resumo de Transcrição IA" if final_meta else "Resumo de Transcrição IA",
                        category="Resumo"
                    ))
        ...
        if config.get("extractChunks"):
            ...
            for i, chunk in enumerate(chunks):
                final_meta = f"{metadata_val} | {chunks_meta}" if metadata_val and chunks_meta else (chunks_meta or metadata_val)
                items_to_add.append(KnowledgeItemModel(
                    knowledge_base_id=kb_id,
                    question=f"Trecho de Conhecimento {i+1}",
                    answer=chunk["text"],
                    metadata_val=final_meta or "Chunk Transcrição",
                    category="Chunking"
                ))
...
@app.post("/knowledge-bases/{kb_id}/process-batch-transcription", dependencies=[Depends(verify_api_key)])
async def process_batch_transcription_endpoint(
    kb_id: int, 
    request: dict, # { "texts": [str], "config": dict }
    db: AsyncSession = Depends(get_db)
):
    """
    Processa múltiplos textos de uma vez e gera um ÚNICO resumo unificado.
    """
    ...
    try:
        # Concatenar todos os textos com separadores claros
        combined_text = "\n\n--- INICIO DO VIDEO ---\n\n".join(texts)
        
        # Geração de Resumo IA Unificado
        try:
            from agent import get_openai_client
            client = get_openai_client()
            if not client:
            ...
            response = await client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "Você é um assistente especialista em síntese de informações. Recebeu transcrições de MÚLTIPLOS vídeos. Resuma todos eles em um único resumo unificado, organizado por tópicos ou cronologia se fizer sentido. Use Português do Brasil."},
                    {"role": "user", "content": f"Textos para resumir coletivamente:\n\n{combined_text}"}
                ],
                temperature=0.5
            )
            summary_text = response.choices[0].message.content
            
            # Criar o item de conhecimento
            new_item = KnowledgeItemModel(
                knowledge_base_id=kb_id,
                question=f"Resumo Unificado de {len(texts)} Vídeos",
                answer=summary_text,
                metadata_val=f"{metadata_val} | Source: Resumo em Lote IA" if metadata_val else "Resumo em Lote IA",
                category="Resumo"
            )
            
            # Gerar embedding
            from rag_service import get_batch_embeddings
            embeddings, _ = await get_batch_embeddings([f"{new_item.metadata_val} | {new_item.question} | {new_item.answer}"])
            if embeddings:
                new_item.embedding = embeddings[0]
                
            db.add(new_item)
            await db.commit()
            
            return {"message": f"Sucesso! Resumo unificado gerado e adicionado à base com sucesso."}
            
        except Exception as se:
            logger.error(f"Erro ao gerar resumo unificado no RAG: {se}")
            raise HTTPException(status_code=500, detail=f"Erro na IA: {str(se)}")
            
    except Exception as e:
        logger.error(f"Erro no processamento RAG em lote: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Erro no processamento: {str(e)}")
...
@app.post("/api/chat/playground", dependencies=[Depends(verify_api_key)])
async def chat_playground(request: Dict[str, Any]):
    try:
        agent_id = request.get("agent_id")
        user_message = request.get("message")
        history = request.get("history", [])
        mock_vars = request.get("mock_vars", {})
        
        # Obter config do agente
        db = async_session()
        async with db as session:
            ...
            # Processar mensagem (Simulado no playground)
            ...
            response_data = await process_message(
                ...
                history=history,
                is_playground=True,
                override_vars=mock_vars
            )

            # Log financeiro do playground
            cost_usd = response_data.get("cost_usd", 0) or 0
            cost_brl = cost_usd * USD_TO_BRL
            
            new_log = InteractionLog(
                agent_id=agent_id,
                session_id=f"playground_{agent_id}",
                user_message=user_message,
                agent_response=response_data.get("content"),
                model_used=response_data.get("model", agent.model),
            ...
            return {
                ...
                "cost_brl": cost_brl,
                "tokens": response_data.get("input_tokens", 0) + response_data.get("output_tokens", 0)
            }
    except Exception as e:
        print(f"❌ Erro no Playground: {e}")
        raise HTTPException(status_code=500, detail=str(e))

def calculate_ai_cost(model_name: str, input_tokens: int, output_tokens: int):
...
@app.post("/api/prompt/advisor", dependencies=[Depends(verify_api_key)])
async def prompt_advisor(request: PromptAdvisorRequest):
    try:
        logger.info(f"🔮 Advisor: Iniciando análise para query: '{request.user_query[:50]}...'")
        # Contexto completo para o Consultor
        context = (
            ...
            "Sua missão é analisar o prompt acima e sugerir melhorias práticas.\n\n"
            "PROMPT PARA ANÁLISE:\n"
            "---\n"
            f"{request.prompt_content}\n"
            "---\n\n"
            f"MENSAGEM INICIAL CONFIGURADA: {request.initial_message or 'Não configurada'}\n"
            f"PERGUNTA INICIAL CONFIGURADA: {request.initial_question_message or 'Não configurada'}\n\n"
            "HISTÓRICO DA CONVERSA:\n"
            f"{json.dumps(request.history, indent=2)}\n\n"
            "INSTRUÇÕES:\n"
            "1. Se o usuário mandar apenas uma saudação (ex: 'oi', 'olá', 'oie'), responda apenas com algo educado como 'Olá! Sou seu Consultor de Prompt. Como posso ajudar você hoje?' e NÃO faça a análise do prompt ainda.\n"
            ...
            "5. Se o usuário solicitar uma alteração ou remoção direta (ex: 'remova X', 'mude Y'), instrua-o a escrever detalhadamente o que deseja no chat e então clicar no botão 'Refinar Prompt Automaticamente' para que a mudança seja aplicada ao editor."
        )
        ...
        response = await client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "system", "content": context}, {"role": "user", "content": request.user_query}],
            temperature=0.3
        )
        ...
        # Log financeiro (Tratado como não-crítico)
        try:
            db = async_session()
            async with db as session:
                new_log = InteractionLog(
                    agent_id=None, # Consultor é global
        ...
        logger.error(f"💥 Advisor: Erro crítico no Consultor: {str(e)}", exc_info=True)
...
@app.post("/api/prompt/refine", dependencies=[Depends(verify_api_key)])
async def refine_prompt(request: PromptRefineRequest):
    try:
        ...
        print(f"❌ Erro ao refinar prompt: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_agent(config: AgentConfig, db: AsyncSession = Depends(get_db)):
    ...
    return AgentConfig(
        ...
        initial_message=db_config.initial_message,
        initial_question_message=db_config.initial_question_message,
        initial_ignore_message=db_config.initial_ignore_message,
...
@app.post("/agents/{agent_id}/generate-description", dependencies=[Depends(verify_api_key)])
async def generate_agent_description(agent_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(AgentConfigModel).where(AgentConfigModel.id == agent_id))
    db_config = result.scalars().first()
    if not db_config:
        raise HTTPException(status_code=404, detail="Agent not found")
    ...
    if not system_prompt:
        raise HTTPException(status_code=400, detail="Agent has no system prompt to analyze")
    ...
    try:
        response = await client.chat.completions.create(
            model=db_config.router_simple_model or "gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Você é um assistente especializado em descrever Agentes de IA baseados em seus prompts. Analise o prompt e escreva uma breve descrição (máximo 2 a 3 frases) indicando para que serve o agente e em quais situações ele deve ser utilizado. Seja direto e não use formatação desnecessária."},
                {"role": "user", "content": f"Prompt do Agente:\n\n{system_prompt}"}
            ],
            temperature=0.3,
        ...
        logger.error(f"Erro ao gerar descrição com GPT: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
@app.post("/agents/bulk-delete", dependencies=[Depends(verify_api_key)])
async def bulk_delete_agents(request: BulkAgentDeleteRequest, db: AsyncSession = Depends(get_db)):
    if not request.agent_ids:
        return {"message": "Nenhum agente selecionado."}
    
    from sqlalchemy import delete
    await db.execute(
        delete(AgentConfigModel)
        .where(AgentConfigModel.id.in_(request.agent_ids))
    )
    await db.commit()
    return {"message": f"{len(request.agent_ids)} agentes removidos com sucesso."}
...
@app.get("/api/chatwoot/labels", dependencies=[Depends(verify_api_key)])
async def get_chatwoot_labels(db: AsyncSession = Depends(get_db)):
    """Busca as etiquetas disponíveis no Chatwoot."""
    url = os.getenv("CHATWOOT_URL")
    token = os.getenv("CHATWOOT_API_TOKEN")
    account_id = os.getenv("CHATWOOT_ACCOUNT_ID")
    
    if not url or not token:
        # Fallback: tenta pegar do primeiro WebhookConfigModel que tenha token
    ...
    if not url or not token:
        logger.warning("Chatwoot credentials not found for label fetching.")
        return []
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # Se não tiver account_id, tenta descobrir via profile (mais compatível que /accounts)
            if not account_id:
                prof_resp = await client.get(f"{url}/api/v1/profile", headers=headers, timeout=10.0)
                if prof_resp.status_code == 200:
                    profile_data = prof_resp.json()
                    # Pega a primeira conta associada ao usuário
                    accounts = profile_data.get("accounts", [])
                    if accounts:
                        account_id = accounts[0].get("id")
            
            if not account_id:
                logger.error("Could not determine Chatwoot Account ID.")
                return []
            
            # 2. Buscar etiquetas da conta
            ...
            if labels_resp.status_code != 200:
                logger.error(f"Chatwoot Error listing labels: {labels_resp.status_code} - {labels_resp.text}")
                return []
            
            data = labels_resp.json()
            payload = data.get("payload", [])
            # Retorna apenas os títulos das etiquetas
            return [l["title"] for l in payload if "title" in l]
    except Exception as e:
        logger.error(f"Exception fetching Chatwoot labels: {str(e)}")
        return []

@app.post("/provision-tools", dependencies=[Depends(verify_api_key)])
async def provision_native_tools(db: AsyncSession = Depends(get_db)):
    """Sincroniza todas as ferramentas nativas do sistema no banco de dados."""
...
async def update_tool(tool_id: int, tool: ToolCreate, db: AsyncSession = Depends(get_db)):
    ...
    logger.info(f"TOOL SAVE [{db_tool.name}]: Add={db_tool.labels_to_add}, Remove={db_tool.labels_to_remove}")
    
    try:
        await db.commit()
        await db.refresh(db_tool)
    except Exception as e:
        logger.error(f"Error committing tool update: {e}")
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    
    return db_tool

@app.get("/tools/{tool_id}/logs", dependencies=[Depends(verify_api_key)])
async def get_tool_logs(tool_id: int, page: int = 1, page_size: int = 20, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ToolModel).where(ToolModel.id == tool_id))
    tool = result.scalars().first()
    if not tool:
        raise HTTPException(status_code=404, detail="Tool not found")
    ...
    for ev in events:
        steps = []
        try:
            raw = json.loads(ev.processing_steps or "[]")
            for s in raw:
        ...
        logs.append({
            ...
            "contato_nome": ev.contato_nome,
            "status": ev.status,
            "steps": steps,
        })

    return {"tool_name": tool.name, "page": page, "logs": logs}
...
async def execute_agent(
    ...
    result = await process_message(
        ...
        ctx, 
        db=db,
        image_url=request.image_url # Pass image token to agent
    )
        ...
        try:
            log_entry = InteractionLog(
                ...
                debug_info=json.dumps({
                    **(result.get("debug") or {}),
                    "image_url": request.image_url, # SAVE IMAGE URL FOR HISTORY
                    "mini_tokens": {"in": getattr(usage, "mini_prompt", 0) if usage else 0, "out": getattr(usage, "mini_completion", 0) if usage else 0},
                    "main_tokens": {"in": getattr(usage, "main_prompt", 0) if usage else 0, "out": getattr(usage, "main_completion", 0) if usage else 0},
            ...
            for call in performed_calls:
                ...
                is_support_tool = any(x in call_name for x in ["suporte", "atendente", "humano", "transbordo", "ajuda"])
            ...
            if support_triggered:
                ...
                if not chk.scalars().first():
                    from agent import generate_handoff_summary, extract_custom_variables, sanitize_phone_number
                    ...
                    u_phone = ctx.get("whatsapp_phone") or ctx.get("phone") or ctx.get("telefone") or ctx.get("user_phone")
                    ...
                    # --- UNIFICAÇÃO E PRIORIDADE DO TELEFONE ---
                    # 1. Prioriza o telefone do contexto/interface (n8n/WhatsApp/Variáveis)
                    # 2. Fallback para extração da IA na conversa se não houver no contexto
                    # 3. Sanitiza (apenas dígitos)
                    extracted_phone = extra_vals.get("telefone") or extra_vals.get("phone") or extra_vals.get("user_phone")
                    
                    final_phone = "N/A"
                    if u_phone:
                        # Prioridade absoluta: Valor fornecido na interface ou via API/Webhook
                        final_phone = sanitize_phone_number(str(u_phone))
                        print(f"📱 Telefone prioritário (Interface/Contexto): {final_phone}")
                    elif extracted_phone:
                        # Fallback: Extraído pela IA na conversa
                        final_phone = sanitize_phone_number(str(extracted_phone))
                        print(f"📱 Telefone fallback (Extração IA): {final_phone}")
                    
                    extra_vals["user_phone"] = final_phone

                    # Refine Reason: prioritize 'motivo' from extracted data or history
                    final_reason = extra_vals.get("motivo") or extra_vals.get("problema")
                    if not final_reason:
                        # Fallback: find the last user message that likely contains the reason
                        for msg in reversed(full_h):
                            if msg["role"] == "user" and len(msg["content"]) > 5:
                                final_reason = msg["content"]
                                break
                    
                    if not final_reason:
                        final_reason = support_reason
    ...
    if usage:
        ...
        print("\n" + "="*50)
        print("📊 RELATÔRIO DE EXECUÇÃO DETALHADO")
        print("="*50)
...
- 50-60%: Cliente expressando dificuldade para usar um produto (ISSO É NORMAL, NÃO DIMINUA A NOTA SÔ POR ISSO).
...
REGRA DE OURO: Se o cliente disser "não consigo fazer isso" ou "é difícil", mas o tom não for de xingamento contra o agente, mantenha a nota de paciência alta (ex: 60-70), pois ele está sendo apenas vulnerável e pediu ajuda, e NÃO que perdeu a paciência com você. Retorne APENAS um JSON: {"sentiment": <0-100>}"""
...
@app.put("/global-variables/key/{key}", dependencies=[Depends(verify_api_key)])
async def update_variable_by_key(key: str, data: dict, db: AsyncSession = Depends(get_db)):
    """Atualiza ou cria uma variável global pelo nome da chave."""
    ...
    if not var:
        var = GlobalContextVariableModel(
            ...
            value=str(val), 
            type="string",
            description=f"Variável gerada automaticamente: {key}"
        )
        db.add(var)
    else:
        var.value = str(val)
...
@app.post("/unanswered-questions/{question_id}/answer-to-prompt")
async def answer_to_prompt(
    question_id: int, 
    req: AnswerToPromptRequest, 
    db: AsyncSession = Depends(get_db), 
    _: None = Depends(verify_api_key)
):
    try:
        # 1. Buscar pergunta e agente
        q_result = await db.execute(select(UnansweredQuestionModel).where(UnansweredQuestionModel.id == question_id))
        question_record = q_result.scalar_one_or_none()
        if not question_record:
            raise HTTPException(status_code=404, detail="Pergunta não encontrada.")
            
        a_result = await db.execute(select(AgentConfigModel).where(AgentConfigModel.id == req.agent_id))
        agent_record = a_result.scalar_one_or_none()
        if not agent_record:
            raise HTTPException(status_code=404, detail="Agente não encontrado.")

        # 2. Preparar texto para a seção FAQ
        final_question = req.question if req.question else question_record.question
        new_entry = f"\n# {final_question}\n\n{req.answer}\n"
        
        current_prompt = agent_record.system_prompt or ""
        header = "## FAQ Adicional"
        
        if header in current_prompt:
            # Insere logo após o cabeçalho
            parts = current_prompt.split(header, 1)
            agent_record.system_prompt = parts[0] + header + new_entry + parts[1]
        else:
            # Cria a seção no final
            agent_record.system_prompt = current_prompt.rstrip() + f"\n\n{header}\n" + new_entry

        # 3. Marcar como respondida e salvar
        question_record.status = "RESPONDIDA"
        question_record.updated_at = datetime.utcnow()
        
        await db.commit()
        return {"success": True, "message": "Pergunta adicionada ao prompt do agente com sucesso."}
    except Exception as e:
        await db.rollback()
        logger.error(f"Error answering to prompt {question_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
@app.get("/support-requests/{support_id}/errors", dependencies=[Depends(verify_api_key)])
async def get_support_request_errors(support_id: int, db: AsyncSession = Depends(get_db)):
    """Busca eventos de erro relacionados a uma solicitação de suporte."""
    req = await db.get(SupportRequestModel, support_id)
    if not req:
        raise HTTPException(status_code=404, detail="Solicitação não encontrada")
    
    # Tenta pegar o telefone dos dados extraídos
    phone = req.extracted_data.get("user_phone") or req.extracted_data.get("telefone") or req.extracted_data.get("phone")
    if not phone:
        return []
        
    # Busca eventos com erro para este telefone
    query = select(WebhookEventModel).where(
        WebhookEventModel.telefone == str(phone),
        WebhookEventModel.status == "error"
    ...
    return [
        {
            ...
            "legenda": ev.legenda,
            "processing_steps": ev.processing_steps,
...
async def resolve_support_request(support_id: int, db: AsyncSession = Depends(get_db)):
    """Marca uma solicitação de suporte como resolvida (remove da fila) e dispara webhook se configurado."""
    return await internal_resolve_support(support_id, db, "ADMIN")

async def internal_resolve_support(support_id: int, db: AsyncSession, resolution_type: str = "ADMIN"):
    """Lógica centralizada para finalizar suporte, disparar etiquetas de robô e webhooks."""
    ...
    # --- [NOVO] ACIONAR FERRAMENTA 'transferir_robo' AUTOMATICAMENTE ---
    try:
        from models import ToolModel, WebhookConfigModel, agent_tools
        import json as _json
        import os as _os
        import httpx as _httpx
        
        # 1. Buscar a ferramenta 'transferir_robo' vinculada a este agente
        tool_stmt = (
            select(ToolModel)
            .join(agent_tools, ToolModel.id == agent_tools.c.tool_id)
            .where(agent_tools.c.agent_id == req.agent_id)
            .where(ToolModel.name == "transferir_robo")
        )
        tool_res = await db.execute(tool_stmt)
        robo_tool = tool_res.scalar_one_or_none()
        
        if robo_tool:
            logger.info(f"🤖 Acionando automação de retorno ao robô para requisição {support_id}")
            
            # A. Reativar automação na tabela de leads
            config_stmt = select(WebhookConfigModel).where(WebhookConfigModel.agent_id == req.agent_id).limit(1)
            config_res = await db.execute(config_stmt)
            config_obj = config_res.scalar_one_or_none()
            
            if config_obj and config_obj.leads_table:
                # O session_id ou extra_data['user_phone'] deve conter o telefone
                target_phone = req.extracted_data.get("user_phone")
                if not target_phone:
                    # Fallback: tenta limpar o session_id se for formato de telefone
                    target_phone = re.sub(r"\D", "", str(req.session_id))
                ...
                if target_phone:
                    ...
                    logger.info(f"✅ Automação reativada para {target_phone}")

            # B. Gerenciar etiquetas no Chatwoot
            account_id = req.account_id
            conversation_id = req.conversation_id
            cw_url = (config_obj.chatwoot_url if config_obj and config_obj.chatwoot_url else _os.getenv("CHATWOOT_URL"))
            cw_token = (config_obj.chatwoot_api_token if config_obj and config_obj.chatwoot_api_token else _os.getenv("CHATWOOT_API_TOKEN"))
            ...
            if cw_url and cw_token and account_id and conversation_id:
                ...
                logger.info(f"🏷️ Sincronizando etiquetas Chatwoot para conta {account_id}, conversa {conversation_id} em {cw_url}")
                
                l_add = _json.loads(robo_tool.labels_to_add) if robo_tool.labels_to_add else []
                l_rm = _json.loads(robo_tool.labels_to_remove) if robo_tool.labels_to_remove else []
                
                async with _httpx.AsyncClient() as cw_client:
                    label_url = f"{cw_url}/api/v1/accounts/{account_id}/conversations/{conversation_id}/labels"
                    # Buscar atuais
                    curr_resp = await cw_client.get(label_url, headers=headers)
                    current_labels = curr_resp.json().get("payload", []) if curr_resp.status_code == 200 else []
                    
                    # Calcular final
                    final_labels = [l for l in current_labels if l not in l_rm]
                    for l in l_add:
                        if l and l not in final_labels: final_labels.append(l)
                    
                    # Aplicar
                    await cw_client.post(label_url, headers=headers, json={"labels": list(set(final_labels))})
                    logger.info(f"🏷️ Etiquetas Chatwoot sincronizadas (Final: {final_labels})")

            # C. Chamar Webhook da Ferramenta (se existir)
            if robo_tool.webhook_url and robo_tool.webhook_url.strip().startswith("http"):
                payload = {
                    "account_id": account_id,
                    "conversation_id": conversation_id,
                    "contact_phone": target_phone, # Usa o telefone identificado
                    "support_id": req.id,
                    "reason": "Retorno ao robô após finalização de suporte humano"
                }
                async with _httpx.AsyncClient() as hw_client:
                    await hw_client.post(robo_tool.webhook_url, json=payload, timeout=10.0)
                    logger.info(f"🚀 Webhook da ferramenta 'transferir_robo' acionado para {robo_tool.webhook_url}")

        else:
            logger.info(f"ℹ️ Nenhuma ferramenta 'transferir_robo' encontrada para o agente {req.agent_id}")

    except Exception as e:
        logger.error(f"⚠️ Erro ao acionar transferir_robo: {e}", exc_info=True)
    
    # --- DISPARO DE WEBHOOK DE FINALIZAÇÃO ---
    try:
        # Busca a URL do webhook configurada globalmente
        var_stmt = select(GlobalContextVariableModel).where(GlobalContextVariableModel.key == "support_finalization_webhook")
        var_res = await db.execute(var_stmt)
        webhook_var = var_res.scalar_one_or_none()
        
        if webhook_var and webhook_var.value and webhook_var.value.strip().startswith("http"):
            webhook_url = webhook_var.value.strip()
            
            # Prepara o payload com os dados solicitados
            payload = {
                "event": "support_resolved",
                "support_id": req.id,
                "session_id": req.session_id,
                "user_name": req.user_name,
                "user_phone": req.extracted_data.get("user_phone") or "N/A",
                "reason": req.reason or "Não informado",
                "account_id": req.account_id,
                "conversation_id": req.conversation_id,
                "resolved_at": req.updated_at.isoformat(),
            ...
            async with httpx.AsyncClient() as client:
                ...
                logger.info(f"✅ Webhook de finalização ({resolution_type}) enviado com sucesso para {webhook_url}")
                
    except Exception as e:
        # Logamos o erro mas não bloqueamos a finalização do ticket no banco
        logger.error(f"⚠️ Erro ao disparar webhook de finalização: {e}")
    # -----------------------------------------

    await db.commit()
    return {"success": True, "message": f"Atendimento finalizado ({resolution_type})."}
...
@app.delete("/support-requests/{support_id}", dependencies=[Depends(verify_api_key)])
async def delete_support_request(support_id: int, db: AsyncSession = Depends(get_db)):
    """Remove permanentemente uma solicitação de suporte sem disparar webhook."""
    result = await db.execute(select(SupportRequestModel).where(SupportRequestModel.id == support_id))
    req = result.scalar_one_or_none()
    if not req:
        raise HTTPException(status_code=404, detail="Solicitação não encontrada.")
...
@app.post("/support-requests/bulk-delete", dependencies=[Depends(verify_api_key)])
async def bulk_delete_support_requests(payload: BulkDeleteSupportRequest, db: AsyncSession = Depends(get_db)):
    """Remove múltiplas solicitações de suporte em massa."""
    from sqlalchemy import delete
    try:
        stmt = delete(SupportRequestModel).where(SupportRequestModel.id.in_(payload.ids))
        await db.execute(stmt)
        await db.commit()
        return {"success": True, "message": f"{len(payload.ids)} solicitações removidas."}
    except Exception as e:
        logger.error(f"Erro ao deletar em massa: {e}")
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
...
@app.get("/public/support/{token}/{session_id}/messages")
async def public_chat_messages(token: str, session_id: str, db: AsyncSession = Depends(get_db)):
    """Acesso público ao histórico de chat de uma sessão."""
    # 1. Validar token público
    var_result = await db.execute(select(GlobalContextVariableModel).where(GlobalContextVariableModel.key == "PUBLIC_ACCESS_TOKEN_SUPPORT"))
    var = var_result.scalar_one_or_none()
    if not var or var.value != token:
        raise HTTPException(status_code=403, detail="Token inválido.")
    
    # 2. Retornar histórico
    history = await get_chat_history(db, 50, session_id)
    return history

@app.patch("/public/support/{token}/{support_id}/resolve")
async def public_resolve_support(token: str, support_id: int, db: AsyncSession = Depends(get_db)):
    """Acesso público para finalizar um atendimento de suporte."""
    # 1. Validar token público
    var_result = await db.execute(select(GlobalContextVariableModel).where(GlobalContextVariableModel.key == "PUBLIC_ACCESS_TOKEN_SUPPORT"))
    var = var_result.scalar_one_or_none()
    if not var or var.value != token:
    ...
    # 2. Reutilizar a lógica centralizada de resolve
    return await internal_resolve_support(support_id, db, "PUBLIC")

    await db.commit()
    return {"success": True, "message": "Atendimento finalizado via acesso público."}
